...
export default class CoveyTownController {
   ...
   */
  private townSocketAdapter(socket: Socket): CoveyTownListener {
    return {
  ...
  connect(token: string, socket: Socket) {
    const session = this._sessions.find(p => p.sessionToken === token);
    if (!session) {
      ...
      socket.on('disconnect', () => this.onDisconnect(listener, session));
